...
const ClientOrderDetailViewModel = (order : Order) => {
    ...
    const getTotal = async () => {
        const calculatedTotal = order.products.reduce((accumulator, product) => accumulator + product.price * product.quantity!, 0);
    ...
    const handlePayment = async () => {
        ...
        if (!values.method) { // Si no se selecciona un método, establecemos el mensaje de error
        ...
        if (result.success) {
            ...
            setOrderGet(getOrder);
            // Actualiza el estado del pedido
            if (getOrder.payment?.method === "Transbank") {
                getTransbankPay(getOrder);
            }
    ...
    const validateTransbank = async () => {
        ...
        if (order.payment?.method === "Transbank") {
            ...
            transbankData = await createTransbankPayment({id_order: values.id_order, total: totalpay.toString()});
        ...
        } else { // setResponseMessage("Error al realizar el pago.");
            console.error("Error al realizar el pago..");
        }
    };

    const getTransbankPay = async(order:Order) => {
        let transbankData = {} as ResponseApiTransbank;
        if (order.payment?.method === "Transbank") {
            await getTotal();
            console.log("TOTAL : ", JSON.stringify(total, null, 3));
            ...
            // Llama al Use Case para obtener el token y la URL de Transbank
            transbankData = await createTransbankPayment({id_order: order.id!, total: totalpay.toString()});
            // Retorna la respuesta
            if (transbankData.success) {
                setTransbankURL(transbankData.data?.url);
                setTransbankToken(transbankData.data?.token);
